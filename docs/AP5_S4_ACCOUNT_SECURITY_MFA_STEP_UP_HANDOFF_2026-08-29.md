...
3. Erfolg darf erst nach erfolgreichem `mfa.unenroll` **und** bestätigtem `refreshSession`/AAL-Abgleich behauptet werden. Verify allein ist kein Erfolg.
4. Abbruch, falscher Code, Challenge-/Verify-Fehler oder AAL-Recheck ohne `aal2` dürfen nicht unenrollen.
5. Refresh-/AAL-Fehler nach bereits erfolgreichem verified Unenroll: Faktor ist weg; lokale Sitzung fail-closed; kein clean success.
6. Factor-/Challenge-/Session-IDs, Tokens und OTP gehören nicht in Nutzertext, URL, Logs oder Analytics.
7. Die Sitzung kann durch den Step-up technisch auf AAL2 angehoben werden. Jetnity aktiviert dadurch keine globale Consumer-MFA-Pflicht.
8. Bei mehreren verified Faktoren wird ein anderer Challenge-Faktor bevorzugt.
9. Login-MFA bleibt skippable. Admin-AAL2 bleibt getrennt. S3-Logout bleibt der lokale Fail-closed-Weg, nicht umgeschrieben.
10. Generation 14 ist nur für AP-5-S4. Nicht für S5 wiederverwenden. Review-Fix `5056084065` bleibt dieselbe Session.
