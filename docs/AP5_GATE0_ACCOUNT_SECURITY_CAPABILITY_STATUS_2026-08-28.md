...
| Unenroll verifizierter Faktoren | **GoTrue verlangt aal2; UI-Step-up in AP-5-S4** | Aktuelle Supabase-Referenz für `auth.mfa.unenroll`: ein Nutzer muss `aal2` haben, um einen **verified** factor zu unenrollen. GoTrue erzwingt das serverseitig. Unverified-factor-Unenroll (z. B. Enroll abbrechen) bleibt ohne AAL2 vorgesehen. |
