...
S5 ist in S5-A (Domainvertrag, dieser Slice) und später S5-B (Persistenz, eigenes Gate) getrennt. S5-A implementiert **keinen** Offer-Monolithen und keine `trip_items`-Spalten. Draft-PR #83: Provider-Refresh nur mit belegter `externalRef`; Current-Quote-Display nur mit belegter `quotedCurrency`. Nicht Ready. Nicht mergen. Kein S5-B.
