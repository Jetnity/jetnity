...
| Frage | Ergebnis |
| --- | --- |
| Wurde ein Current-Password-Vertrag erfunden? | Nein. Explizit verboten. Signed-in Change bleibt `reauthenticate()` → Nonce → `updateUser({ password, nonce })`. Recovery bleibt eine eigene Authority. |
...
| Wurde MFA-Step-up als App-Pflicht oder als `unknown` bei GoTrue belassen? | Review-Fix: Jetnitys UI steppt nicht hoch. GoTrue verlangt für verified-factor Unenroll `aal2` und erzwingt das serverseitig. Das ist nicht mehr `unknown`. |
...
| Wurde behauptet, `current_password = true` breche Recovery sicher? | Nein, nach Review-Fix. Der Wechsel bleibt PO-Gate; Recovery-Kompatibilität muss separat verifiziert werden. |
...
- Ein späterer Agent könnte `/auth/update-password` als In-Account-UI „wiederverwenden“ und dabei Recovery und signed-in Reauthentication vermischen. S2 muss die zwei Authorities getrennt halten.
- Login-MFA-Abbrechen lässt AAL1 stehen. Das ist ein echter Rest, kein AP-5-Runtime-Fix in diesem Slice.

## 4. Urteil des Autors

**CHANGES REQUIRED durch den Autor nach Review `5049870788`:** die drei Auth-Truth-Korrekturen (verified-factor Unenroll `aal2`, Recovery ≠ Reauthentication, Current-Password-Breakage nicht unbelegt) sind in Status, ADR-0182, Handoff, Self-Review, PR-Body und S2/S4-Schnitt nachgezogen. Keine Runtime.
