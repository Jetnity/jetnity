...
Self-expiring: solange #187 offen → Review von `5463879179`. Sobald gemergt → Kern integriert; nächster Schritt zuerst Post-Merge-Verifikation + TL-Continuity, nicht automatisch Skyscanner. Autor setzt kein Ready/Merge.
...
Kommentar `5463879179` gegen Review-Head `ec7eff42`:

- `requestIdHeaderName` muss ein gültiger HTTP-Header-Name sein. Default-Secret-Liste ist `invalid_configuration` bei Executor-Create. Per-Request `additionalSensitiveHeaderNames` (z. B. `x-partner-secret`) ist `invalid_request` vor HTTP, ohne Result/Event-Leak. Unregistrierte Custom-Request-IDs bleiben zulässig.
