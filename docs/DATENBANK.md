...
Gültig für: Supabase-Development-Branch nach Phase 3.1 plus Booking-Status (`20260821100000`), Mobilität (`20260821120000`) und Mietwagen (`20260821200000`, nur Development). Airport- und Places-Schema sowie deren Importe gelten nur für Development, bis der kontrollierte Rollout in [PRODUCTION_ROLLOUT.md](PRODUCTION_ROLLOUT.md) ausdrücklich freigegeben ist.

Zwei getrennte Aussagen, kein Widerspruch: **Production hat** `20260821100000` (PR #29) und `20260821120000` (PR #30) nach ausdrücklicher Nutzerfreigabe. Das Playbook in [PRODUCTION_ROLLOUT.md](PRODUCTION_ROLLOUT.md) hält `20260820130000` als Default-Sicherheitsgrenze für automatische Production-Läufe. Das ist eine Guardrail, kein Gegenbeweis zum realen Production-Stand. `20260821200000` bleibt Development-only und darf nicht auf Production angewendet werden.
...
**Mietwagen `20260821200000`:** Neuer `kind`-Wert `rental_car` plus vier optionale Spalten. Origin/Destination dürfen bei Transfer **oder** Mietwagen stehen; Transfer-Modusfelder bleiben transfer-only. `booked` ist für `flight|stay|transfer|rental_car` zulässig. `reise_anlegen()` schreibt die Felder und setzt Mietwagen-Evidenz nur auf `user`. `reise_aendern()` wird nicht ersetzt. **Development angewendet am 21. August 2026.** Verifiziert: 4 Spalten, CHECKs inkl. `booking` für `rental_car`, 0 Mietwagenzeilen, 0 ungültige Mietwagenfakten auf anderen Arten, 0 Transfer-Modusfelder auf Mietwagen, 0 ungültige gebuchte Nicht-Kommerzielle Zeilen, RLS auf `trip_items` aktiv, `reise_anlegen()` SECURITY INVOKER, `reise_aendern()` schreibt die Spalten nicht. `npm run db:typen -- --pruefen` entspricht dem live Schema. **Nicht Production.**
...
| `20260821200000_trip_items_rental_car.sql` | `kind=rental_car` plus optionale Mietwagenspalten; `reise_anlegen()` schreibt sie; Booking-CHECK erlaubt `booked` für `rental_car` (Foundation B, ADR-0092). **Nur Development. Nicht auf Production anwenden.** |
