...
**Technical-Lead re-reviews on `3775d980`, `74577e31`, `f46d43a0`, `4837fc9a` and `4ffe3f20` — six truth findings, all fixed in this session:**
...
6. That language fix was still a heuristic plus a finite denylist, and it only covered `antwort`: `Das ist so: İtalya için vize gerekli.` satisfied the German-marker heuristic while the Turkish claim appeared in no list. A denylist over an open set is never complete. The contract is now **inverted**: `lib/reisebegleiter/wortschatz.ts` holds an allowlist — the curated German register, every word from the server-derived context (stage/country/traveller names out of the accepted projection), and numbers. Everything else is unattested and rejects the suggestion, in **every** model-written field. German inflection and compounds resolve, with a four-character minimum per compound part so syllable splitting is not a way around. Nothing depends on which languages are enumerated any more. The cost is recall and it is deliberate: a rejected suggestion is a missing feature, an invented entry requirement is harm.
