...
| What it actually checks today | **App-Prozess:** this Next.js process answered. **App-Deployment:** always `unknown` (`source: none`). **Supabase App-Datenzugriff:** on a **cache miss**, a cookie/session client runs `airports` `select iata limit 1` with 8s timeout, only when `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY` are set. A **cache hit** (30s, process-wide in `sammeln.ts`) returns the previous `bericht` after `wendeEvidenceAlterAn` and does **not** run the current caller’s ping. **Supabase Management, Vercel, GitHub/CI, Infomaniak:** `not_configured` / `source: none`. |
| Establishes (collector / Slice B board) | Process reachability of this instance; that named management/CI/DNS sources are not attached; freshness/`checkedAt` of the last process collection. On a miss, that a cookie client in **that** miss could read `public.airports`. Slice B `proves` for success currently says “in dieser Sitzung” — that is **board copy on the miss path**, not a cache-hit or analyst guarantee. |
| Establishes (Foundation 1 analyst) | Only a **process-recent** observation after the `betrieb-lesen` gate. Never “this viewer’s session answered airports.” See decision §6.4. |
| Cannot establish | Current-session provenance of a cached report; that caller B executed the ping caller A cached; break-glass database reachability; deployment or Vercel platform health; Supabase project / Auth / billing / Management; CI green; Infomaniak domain/mail; provider liveness; incidents; revenue; that “all systems are healthy”. `VERCEL_*` metadata is not a health proof. |
| Gate | `betrieb-lesen` + AAL2 on the page and API. Home (`/admin`) currently has **area access only**. A home-mounted analyst **must re-check `betrieb-lesen` before calling `sammleSystemHealth`**. Denied / `lookup-failed` / `aal-lookup-failed`: **no source load**. Do not render an empty “all quiet” list. Break-glass may load the process snapshot but must run the §6.4a projection: no database-backed fact, including cached airports success. Banner text is not that proof. |
| Privilege | Existing read. No new secret, role, RPC, table, cache or permission subsystem. Airports ping (miss only) uses a cookie client, not service role. Management tokens are **not** present and must not be added. |
| Freshness | `berechneFreshness({ checkedAt, nowMs, ttlMs })`: missing/invalid `checkedAt` → `unknown`; `ageMs > ttlMs` → `stale`; else `fresh`. TTL: app/supabase 60s, vercel 120s, github/infomaniak 300s. Cached bericht is re-aged via `wendeEvidenceAlterAn` **without rewriting `checkedAt`**. Isolated collector failures fall back to a typed item, not a thrown empty board. |
| Safe fields | `id`, `name`, `status`, `source`, `checkedAt`, `freshness`, `summary`, `detail`, `doesNotProve`, non-secret metadata. Sub-check ids `app-prozess`, `app-deployment`, `supabase-app-datenzugriff`, `supabase-management`. Analyst may reuse `proves` only after the session-wording overlay. |
| Forbidden | Copying “in dieser Sitzung” into analyst `proves`; attributing cached airports success to break-glass; Management tokens; raw SQL errors as executable text; user/trip/security PII; inventing `healthy` for parents; fabricating `checkedAt`. |
| User-visible use | Role grant: “Supabase App-Datenzugriff is unavailable in the recent process observation; this does not prove Management is down; next: open System Health.” Break-glass: do not use that airports status as a fact. Expected `unknown` / `not_configured`: coverage disclosure, **no invented configure-token recommendation**. |
...
The first useful truthful analyst can be built from **System Health alone**: it is the only existing sanitized snapshot whose statuses actually change under current permissions (`unavailable` / `unknown` / `not_configured` / sub-check `healthy`) and that already carries `doesNotProve` and freshness. Analyst `proves` is process-recent after overlay, because the collector cache is not session-bound. A second source is not required to start. Provider `model-usage` is the named next snapshot, not part of the first runtime task.
