...
### 3.2 Contract / privacy / commercial-control concerns

The current public Wego Affiliate Terms contain material clauses that require dedicated legal/privacy review before any `A-WEGO` submission can become a recommendation to proceed:

- Affiliate Websites must be approved by Wego.
- Users referred through the Affiliate Program who search for travel through Wego are stated to be **customers of Wego**.
- The Terms state that Wego will own information generated by those users and that Wego rules/policies relating to those users, including personally identifiable information, apply.
- Wego reserves the ability to vary referral commercial terms; the Terms state that some referrals may be temporarily or permanently non-monetized.
- The initial term is stated as one year and then automatically renews for recurring one-year periods unless terminated according to the agreement.
- The public Terms describe termination notice at least 30 days before the end of the term and require removal of Wego brand/data on termination.

These clauses do not automatically make Wego unusable, but they are material for Jetnity because Jetnity's privacy and traveller architecture is designed around explicit data ownership, minimisation and truthful provider boundaries.

Required before any Wego approval:

1. legal interpretation of the user-information ownership clause for Jetnity's intended API flow;
2. exact controller/processor roles and DPA/privacy terms for Swiss/EEA users;
3. data fields transferred to Wego and whether route/search data becomes Wego-controlled user information;
4. retention/deletion/export obligations;
5. international-transfer/subprocessor terms;
6. exact commercial-change / non-monetized-referral mechanism;
7. current termination rights and whether separate API terms supersede/modify the public Affiliate Terms.

Until reviewed:

> **Wego privacy/commercial contract fit = MATERIAL OPEN RISK.**
...
- Wego's current public user-information ownership wording makes pre-contract invention of a Jetnity/Wego analytics persistence model especially unsafe.
...
Therefore V1 Step 2 is now genuinely constrained by provider access/commercial/privacy terms, not a missing generic Flight runtime contract.
...
3. map exact questions for rate limits, fee, attribution, cache, persistence, branding, DPA, user-data ownership and Swiss market support;
...
1. **KAYAK** — priority 1 for access due diligence; strong product/affiliate fit and explicit startup/Sandbox posture, but critical production, privacy and commercial terms remain private/unknown.
2. **Wego** — priority 2 technical candidate, but now with **material contractual/privacy risk** in addition to the 5% Search-to-Click covenant and conflicting public pricing evidence; no recommendation to submit until these terms are reviewed.
...
- `A-WEGO`: only after privacy/legal readiness, submit Wego registration/API contact and review the offered current terms;
