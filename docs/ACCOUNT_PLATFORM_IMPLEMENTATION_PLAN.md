...
| Fehlt | In-Account-Oberfläche für die bereits existierende Passwortänderung, ohne den Reauthentication-Vertrag zu ersetzen; ehrliche Session-Karte (`unsupported` für andere Geräte – eine echte Liste gibt der installierte User-Client nicht her); explizite Logout-Scopes `local`/`others` (heutiges Abmelden ist bereits `global`); explizites Step-up vor Unenroll verifizierter Faktoren. Gate-0-Evidence: `docs/AP5_GATE0_ACCOUNT_SECURITY_CAPABILITY_STATUS_2026-08-28.md`, ADR-0182. **Kein Runtime-Start.** |
