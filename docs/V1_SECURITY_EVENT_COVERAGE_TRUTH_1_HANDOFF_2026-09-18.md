...
Status: **TL P2 FIXED ON `9c02d14e` WITH GATES RECORDED / RE-GATE THIS PERSIST HEAD / STOP FOR TECHNICAL-LEAD REVIEW / KEIN READY / KEIN MERGE / KEIN FOLGESLICE**
...
- **P2:** 24h KPIs are computed from unfiltered `data.events`. Search filters only the table.
...
1. `last24` is sourced from `aufgezeichneteEvents` / `data.events`, not from the filtered `events` memo.
2. Table entry count still uses filtered `events.length`.
3. `securityHinweis` still states incomplete coverage, recorded-vs-real distinction, no live monitoring, and blocklist not enforced.
4. Allowed-file boundary held; no #483 files; no global continuity edits.
5. `9c02d14e` CI `35357302461` SUCCESS and Vercel `EfkQKormXNw9qJtoFwZpGs1EKjtK` READY are recorded only for that SHA. behind=0 was verified against live `origin/main@21f489d3`.
