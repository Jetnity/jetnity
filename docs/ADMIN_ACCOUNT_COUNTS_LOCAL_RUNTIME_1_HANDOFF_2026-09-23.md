...
3. Binding TL re-review `5294264491` on `12f42ab2ab2a8a9b67086dece9f0b76fd84f767c` (CHANGES REQUIRED) — this head is the same-session F1–F4 package
4. Independent diagnostic spec comment `5799328807`
...
- Do not duplicate this 5294264491 correction if already present on the current head
- #559 navigation correction is independently accepted at bounded scope and waits; do not import it
...
7. Historical receipts stay dated. Do not rewrite them.
...
| Capability | Code after 5294264491 | Executed here |
| --- | --- | --- |
...
| Sanitized artifact export-before-delete | yes | real temp files |
| Official CLI / Docker / GoTrue / Next / live observer | yes (prior review-fix) | **NOT RUN** |
| Sibling UI G6–G19 | sibling-owned | **NOT RUN** |
| Code-complete claim | **false** | n/a |

## Secrets

Per-run passwords, TOTP, JWT, cookies, service-role and database URLs stay out of Git. Context is in-memory only. Durable receipts are the public evidence folder; private HOME is not recreated after G20.
