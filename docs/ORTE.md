...
Für Rolle `ziel` liest der Länder-Alias-Nachzug das **begrenzte Länder-Universum** (`typ = country`, Limit `ORT_LAND_UNIVERSUM`). Er filtert nicht per Substring-`ilike` mit kleinem Limit: kurze Exact-Tokens wie zwei Buchstaben können sonst hinter vielen Teilstring-Kandidaten verschwinden. Das Universum ist endlich; solange das Limit ≥ der Anzahl Länderzeilen bleibt, kann ein vorhandenes Exact-Alias nicht still abgeschnitten werden. Das konsumiert den bestehenden Alias-Bestand und ist kein hartcodierter Länderkatalog, kein zweiter Suchpfad und kein Geocoder. Rolle `abreise` holt dieses Universum nicht.
