...
Für Rolle `ziel` holt der Länder-Alias-Nachzug **selektiv** nur Länder mit exaktem Namen oder exaktem Komma-Token (`ortLandAliasExaktfilter`). Er lädt nicht das ganze Länder-Universum und nicht per Substring-`ilike %token%`: kurze Exact-Tokens verschwinden sonst hinter Teilstring-Kandidaten, normale Stadt-Queries würden sonst jede Zielsuche mit allen Länderzeilen belasten. Das Limit `ORT_LAND_UNIVERSUM` ist eine Sicherheitskappe, kein Transfer des Bestands. Truncation kann nur eintreten, wenn mehr Länder dasselbe Exact-Token teilen als dieses Limit – das ist dokumentiert, nicht versteckt. Rolle `abreise` holt keinen Länder-Nachzug.
