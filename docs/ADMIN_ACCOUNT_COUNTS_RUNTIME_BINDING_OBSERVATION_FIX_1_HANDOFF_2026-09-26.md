...
3. TL review `5325722210` C1 only
4. Base/main `40fffe38102012ae3ffa5f5b7e2bc24c1a100b42`
...
- Immediate TL re-review of this exact PR/head = **same** logical agent, Generation 1, session `bc-abaf4d85-1737-4b3d-b5d4-802ea7bc697a`, model `cursor-grok-4.6-high-fast`
...
| Capability | Code after C1 | Executed here |
| --- | --- | --- |
...
| Post-start binding observation | NetworkSettings-first + C1 configured-port reconcile | Mailpit-shaped PASS + missing/null/empty/malformed FAIL + unconfigured null ignored |
| Strict loopback policy | unchanged | existing + C1 FAIL cases |
...
Parent Docker credentials/config are not copied. Durable receipts sanitize user home paths. Per-run secrets stay out of Git. Receipt `aaclr1-20260926T111109Z` names a denied parent `SUPABASE_ACCESS_TOKEN` without copying its value.

## Next actor

Technical Lead: independent exact-head re-review of Draft PR #564 after C1. Do not treat helper PASS as real-Mac acceptance. The authorized later Mac rerun remains required.
