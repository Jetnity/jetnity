...
| Trigger / event-insert failure | Fail-closed. Injected BEFORE INSERT fault rolls back source, event and `used`. | R1 `P0001 injected security_events insert fault`; source/tracked/used = 0 |
...
| Cleanup/quota drift | Cleanup and `used` share `FOR UPDATE` on the quota row. Rollback restores both. Drifted `used` disables writes until repaired. | R3 + invalid-state cases |
...
- A later persistent activation that copies this SQL into `supabase/migrations/` without a separate gate would be a scope failure. This PR must not be treated as that gate.
- `service_role` BYPASSRLS can still mutate `blocked_ips`. Null `auth.uid()` emits no event and does not raise. That residual is disclosed, not closed.
- If `service_role` is given a forged JWT `sub`, the trigger would mint a trusted-looking event. That is the same privileged residual class as baseline ALL. Not advertised as the authenticated producer guarantee.
...
- The concurrency proof observed a lock wait on attempt 2 in the passing run. A flaky scheduler could require retries; the command fails if no wait is seen after 3 attempts.
...
| F1/F2/R1/R2/R3/R4 matrix | Yes — 56 harness cases |
