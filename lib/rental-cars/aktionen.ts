// lib/rental-cars/aktionen.ts
//
// Manueller Mietwagen in eine Reise im Konto. Nutzerangabe, kein Providerfakt.
...
export async function rentalCarManuellInReiseAnlegen(eingabe: unknown): Promise<Aktionsergebnis<null>> {
  const rahmen = manuellKontoSchema.safeParse(eingabe)
  if (!rahmen.success) return { ok: false, meldung: 'Diese Reise ist unbekannt.' }

  const gelesen = rentalCarManuellLesen(eingabe)
  ...
  const zugehoerig = rentalZugehoerigkeitPruefen(reise, gelesen.eingabe.dayId, gelesen.eingabe.stageId)
