...
describe('AP-5-S3 Vertrag und Accessibility', () => {
  ...
  test('kein Session-Listing, kein S4/S5 und keine Secret-Logs', () => {
    ...
    assert.equal(mfa.includes('accountLogoutScopeAction("others")') || mfa.includes("accountLogoutScopeAction('others')"), false)
    assert.equal(mfa.includes('accountLogoutScopeAction("global")') || mfa.includes("accountLogoutScopeAction('global')"), false)
