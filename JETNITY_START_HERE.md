...
> **While Draft PR #491 is open, docs on `main` are not sufficient.** A new chat must inspect the live PR #491 head, comments, and evidence, then read `docs/CHATGPT_NEW_CHAT_CHECKPOINT_2026-09-18.md` §0 (Exact OS-2 continuation point). The next exact external step is the CoS read-only capability/routing inventory. Cursor does not manufacture a Guardian result. ChatGPT / Technical Lead alone decides PASS / Ready / Merge.
