...
> **While Draft PR #491 is open, docs on `main` are not sufficient.** A new chat must inspect the live PR #491 head, comments, and evidence, then read `docs/CHATGPT_NEW_CHAT_CHECKPOINT_2026-09-18.md` §0 (Exact OS-2 continuation point). The next exact external step is the same Jetnity Chief of Staff receipt-evidence capture, then Technical-Lead evidence assessment, while awaiting the existing 2026-09-20 07:45 Daily. Cursor does not implement that work. ChatGPT / Technical Lead alone decides PASS / Ready / Merge.
