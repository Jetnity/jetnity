...
> **While Draft PR #491 is open, docs on `main` are not sufficient.** A new chat must inspect the live PR #491 head, comments, and evidence, then read `docs/CHATGPT_NEW_CHAT_CHECKPOINT_2026-09-18.md` §0 (Exact OS-2 continuation point). The next exact step is Technical-Lead exact-head review of this persist. Sep20 Daily already happened and is recorded. Await the existing 2026-09-21 08:30 Weekly as a **schedule**, not a receipt. Cursor does not implement that work. ChatGPT / Technical Lead alone decides PASS / Ready / Merge. #491 must not set `NORMAL`.
