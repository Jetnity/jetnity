-- LOCAL-ONLY staged/unexposed postcondition. Shared identity must be REVOKED_EXACT.
...
BEGIN
  ...
  IF klass ->> 'state' IS DISTINCT FROM 'REVOKED_EXACT' THEN
    RAISE EXCEPTION 'jetnity.rollout-prep.v1: verify expected REVOKED_EXACT, got %', klass
      USING ERRCODE = 'XX000';
  END IF;
