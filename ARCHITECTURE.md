...
Duffel ist der erste Datenadapter, nicht die Produktarchitektur. Search und Booking/Affiliate sind getrennt; `booking_url` bleibt `null`. Ein späterer Skyscanner- oder Aviasales-Adapter implementiert dasselbe `FlightProvider`-Interface. Amadeus Self-Service ist eingestellt und nicht angebunden. Seit E5-B1 darf ein strukturiertes Duffel-Airport-Objekt sein explizites `time_zone` als optionale `departureTimezone` / `arrivalTimezone` in `FlugSegment` und die trusted Route-Itinerary tragen (ADR-0207). String-/IATA-only, fehlende oder ungültige Zone bleibt `null`. Lokale Flight-Datum-/Uhrzeit bleiben Wall-Clock-Werte; es gibt keine IANA→UTC-Umrechnung und keinen Event-Resolver. Untrusted Browser-/Guest-Itineraries können Timezone nicht etablieren. Die Live-SQL-Funktion `flug_route_itinerary_metadata` kanonisiert weiterhin ohne Timezone-Felder; eine Trigger-Erhaltung wäre eine spätere Migration.
