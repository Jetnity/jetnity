...
Draft-PR #34, keine Schemaänderung. `lib/route/` leitet `RouteFacts` nur aus validierten Flight-Itineraries ab. Persistenz nutzt vorhandenes `trip_items.metadata` als `{ routeItinerary }` (max. 8192 Zeichen). Die Flugsuche löst IATA-Länder in einem Batch gegen `public.airports` auf; die Konto-Übernahme löst erneut serverseitig auf. Guest und Account teilen dasselbe `TripItem.routeItinerary`. Die UI zeigt die Route im Flugbereich progressiv und in der Übersicht dezent; keine neue Hauptnavigation. Connection Duration und Airport Change entstehen nur aus strukturierten Segmenten, nicht aus Titeln. Production-Suche und Timatic bleiben aus.
