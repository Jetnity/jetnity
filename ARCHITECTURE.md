...
`routeFactsAusReise()` ist die einzige Origin-/Transit-Naht. Foundation D füllt sie aus `trip_items.metadata.routeItinerary`; ohne Itinerary bleibt `quelle: 'none'`. Ortsnamen und Place-IDs werden nicht in Ländercodes geraten (ADR-0108, ADR-0112). Fachlich: [docs/TRAVEL_READINESS.md](docs/TRAVEL_READINESS.md), [docs/ROUTE_TRANSIT_INTELLIGENCE.md](docs/ROUTE_TRANSIT_INTELLIGENCE.md), ADR-0096 bis ADR-0112.

### Route & Transit Intelligence (Foundation D)

Draft-PR #34. `lib/route/` leitet `RouteFacts` nur aus validierten Flight-Itineraries ab. Persistenz nutzt vorhandenes `trip_items.metadata` als `{ routeItinerary }` (max. 8192 Zeichen). `reise_anlegen()` schreibt die validierte Itinerary auf Development atomar in derselben Transaktion (ADR-0113); der TypeScript-Nachlauf ist fail-closed Recovery. Die Flugsuche löst IATA-Länder in einem Batch gegen `public.airports` auf; die Konto-Übernahme löst erneut serverseitig auf. Guest und Account teilen dasselbe `TripItem.routeItinerary`. Die UI zeigt die Route im Flugbereich progressiv und in der Übersicht dezent; keine neue Hauptnavigation. Connection Duration und Airport Change entstehen nur aus strukturierten Segmenten, nicht aus Titeln. Production-Suche, Production-Migration und Timatic bleiben aus.
