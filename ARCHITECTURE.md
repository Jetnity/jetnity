...
AP-1–AP-4 sind integriert und werden nicht neu geplant. AP-5 Gate 0 rekonstruiert den bestehenden Auth-/Session-/MFA-Vertrag (ADR-0182) und ist integriert. AP-5-S1 macht `/account/security` ehrlich (`empty` ≠ `unsupported` ≠ `unavailable` ≠ `error`; Passkeys folgen Server-Config; keine Roh-GoTrue-Copy; ADR-0183). AP-5-S2 ist integriert (PR #137). Nur AP-5-S3–S5 bleiben ungebaut und sind nicht automatisch gestartet. AP-6 und AP-8–AP-12 bleiben ungebaut/gated wie im kanonischen Plan. Current Traveller Truth einer konkreten Reise bleibt trip-scoped Foundation E. Dual-Authority ist product-owner-freigegeben: Account Registry = wiederverwendbare Identität/Fakten, Trip Snapshot = einzige Trip-Current-Truth. AP-7 Gate 0 ist integrierte Architecture-Evidence (PR #144 / ADR-0186). AP-7-S1 (Draft-PR #145 / ADR-0187) ist der shared Domain-Contract ohne Schema/Persistenz/UI. Production-Migration, RLS/Identity und AP-7-S2 bleiben separat gegated. Die historische Datei auf Draft-PR #39 ist keine Current Truth.
