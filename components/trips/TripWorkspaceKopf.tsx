...
function etappenText(reise: Trip) {
    ...
    return (
      <section className="mt-5 rounded-[24px] bg-brand-800 px-4 py-4 text-white shadow-[0_18px_50px_rgba(15,46,42,0.14)] sm:px-5">
        ...
        <p className="mt-1.5 flex min-w-0 items-start gap-2 text-sm text-white/70">
          ...
          <span className="min-w-0 hyphens-auto break-words">{etappenText(reise)}</span>
        </p>
        <p className="mt-3 text-xs leading-5 text-white/70">
          <span>{zeitraum(reise)}</span>
          <span aria-hidden="true"> · </span>
          <span>
            {reise.travellers} {reise.travellers === 1 ? 'Reisende Person' : 'Reisende'}
          </span>
          <span aria-hidden="true"> · </span>
          <span>Budget {betrag(reise.budgetAmount, reise.currency)}</span>
        </p>
