...
export default function TrainSearchForm({ onSubmit }: TrainSearchFormProps) {
  ...
  function submit(e: React.FormEvent) {
    ...
    onSubmit?.({ from, to, datetime: dt, passengers: pax, classType, directOnly, flexible })

    if (!onSubmit) {
      const p = new URLSearchParams()
      p.set('from', from.trim()); p.set('to', to.trim()); p.set('date', date); if (time) p.set('time', time)
      p.set('adults', String(pax.adults)); p.set('children', String(pax.children)); p.set('class', classType)
      if (directOnly) p.set('nonstop', '1'); if (flexible) p.set('flex3', '1')
  ...
  return (
    <form onSubmit={submit} className="rounded-2xl bg-white/95 p-4 text-[#0c1930] shadow-inner md:p-5">
      {/* Top row: Von | Swap | Nach | Datum | Zeit | Pax | Suchen */}
      <div
        className={cn(
          'grid grid-cols-1 items-end gap-3',
          'md:[grid-template-columns:1.2fr_2.5rem_1.2fr_0.95fr_0.75fr_1.2fr_auto]'
        )}
      >
        ...
        <Field className="md:[grid-column:5/6]" label="Zeit (optional)">
          <input type="time" value={time} onChange={(e) => setTime(e.target.value)} className="w-full bg-transparent text-sm outline-none" />
        </Field>
...
function PaxPicker({ value, onChange, label }: { value: Pax; onChange: (v: Pax) => void; label: string }) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div role="dialog" aria-label="Passagiere" className="absolute right-0 z-[1001] mt-2 w-[24rem] max-w-[95vw] rounded-2xl border border-zinc-200 bg-white p-4 shadow-xl">
