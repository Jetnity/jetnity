...
export default function ActivitySearchForm({ onSubmit }: ActivitySearchFormProps) {
  ...
  return (
    <form onSubmit={submit} className="rounded-2xl bg-white/95 p-4 text-[#0c1930] shadow-inner md:p-5">
      {/* Top row: Ziel | Datum | Reisende | Suchen */}
      <div
        className={cn(
          'grid grid-cols-1 items-end gap-3',
          'md:[grid-template-columns:1.6fr_1.1fr_1.3fr_auto]'
        )}
      >
        <Field className="md:[grid-column:1/2]" label="Ziel / Stadt">
          <input
            value={dest}
        ...
        <Field className="md:[grid-column:2/3]" label="Datum (optional)">
          <input
            type="date"
            value={date}
      ...
      {/* Secondary row: Kategorien + Filter + Flex */}
      <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-12">
        <div className="md:col-span-8">
          <span className="mb-1 block text-xs font-medium text-zinc-500">Kategorien (optional)</span>
