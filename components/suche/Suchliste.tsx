        ...
        eintraege.map((eintrag, index) => (
          <li
            ...
            className={cn(
              ...
              eintrag.betont && aktiv !== index ? 'bg-surface-50' : null,
              aktiv === index ? 'bg-surface-100' : 'hover:bg-surface-25',
            )}
          >
            <span className="min-w-0">
              <span className={cn('block text-brand-800', eintrag.betont ? 'font-semibold' : 'font-medium')}>
                {eintrag.primaer}
              </span>
              {eintrag.sekundaer && (
                <span className="mt-0.5 block text-xs leading-5 text-ink-700">{eintrag.sekundaer}</span>
              )}
            </span>
            {eintrag.extra && (
              <span
                className={cn(
                  ...
                  eintrag.betont ? 'bg-surface-100 text-brand-800' : 'bg-surface-50 text-ink-800',
                )}
              >
                {eintrag.extra}
              </span>
