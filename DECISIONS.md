...
8. **Der Wortschatz einer Auskunft ist eine Erlaubnisliste, keine Verbotsliste.** Die Prüfung aus Punkt 7 liest deutsche Muster. Drei Fassungen haben versucht, erfundene amtliche Wahrheit an ihren Wörtern zu *verbieten* – erst auf Deutsch, dann in den von Jetnity geführten Sprachen, dann über Markerwörter als Spracherkennung. Jede war eine Verbotsliste über einer offenen Menge und damit widerlegbar: „Das ist so: İtalya için vize gerekli.“ trug genug deutsche Marker, um für deutsch zu gelten, und eine Behauptung in einer Sprache, die in keiner Liste stand. Über einer offenen Menge gibt es keine vollständige Verbotsliste; es gibt nur eine vollständige Erlaubnisliste. `lib/reisebegleiter/wortschatz.ts` führt deshalb den Register, den die Systemregeln verlangen. Zulässig ist er, dazu jedes Wort aus dem **serverseitig abgeleiteten** Kontext (Etappen-, Länder- und Reisendennamen aus der akzeptierten Projektion) und Zahlen. Deutsche Flexion und Komposita lösen auf; jeder Kompositumsteil braucht mindestens vier Zeichen und einen geführten Stamm, damit Silbenzerlegung kein Umgehungsweg wird. Die Schranke gilt für **jedes** modellgeschriebene Feld – eine erfundene Anforderung wirkt in einer Liste wie in einem Satz. Der Preis ist Recall und er ist bewusst bezahlt: Eine abgelehnte Auskunft ist ein ausgefallenes Merkmal, eine erfundene Einreiseanforderung ist ein Schaden. Antwortsprache bleibt Deutsch (`COUNTRY_UI_LOCALE`); die Zeile „in der Sprache der Frage“ war eine Ausweitung dieses Slice selbst und ist zurückgenommen.
...
**Bilanz der sechs Runden:** Fünf Korrekturen haben den Geltungsbereich erweitert – vom Kontext auf den genannten Bezug, auf den Anforderungstyp, auf die Gesamttaxonomie und die Aussagerichtung, dann auf die Sprachfläche. Die sechste hat den Ansatz getauscht: von „welche Wörter sind verboten?“ auf „welche Wörter sind erlaubt?“. Die Lehre für vergleichbare Wahrheitsschranken: Die Frage ist nicht „schliesst diese Regel den genannten Fall?", sondern „über welche Dimension lässt sie sich umgehen?" – und die letzte Dimension war nicht der Inhalt, sondern die Zuständigkeit.
