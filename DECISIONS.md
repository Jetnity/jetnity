...
4. **Das Modell schreibt keine Sätze. Es wählt aus.** `lib/reisebegleiter/schema.ts` hat **kein Freitextfeld**. Drei Felder auf oberster Ebene, alle nur Schlüssel und Bezugskennungen:

   | Feld | Inhalt |
   | --- | --- |
   | `befunde` | Paare aus Katalogschlüssel und Bezug (`lib/reisebegleiter/befunde.ts`) |
   | `bezuege` | Kennungen, deren Jetnity-Stand die Oberfläche daneben zeigt |
   | `amtlicheHinweise` | Paare aus Official-Bezug und Aussageschlüssel (`lib/reisebegleiter/aussagen.ts`) |

   Jeden angezeigten Satz schreibt **Jetnity**. Damit sind eine erfundene amtliche Anforderung, ein Betrag, ein Link, eine Verfügbarkeit, eine Buchungsaussage und eine behauptete Reiseänderung nicht *abgelehnt*, sondern **nicht darstellbar** – es gibt kein Feld, in das sie passen. `z.strictObject` plus `additionalProperties: false` lehnen zusätzliche Felder ab, statt sie zu entfernen; `lib/reisebegleiter/schema.test.ts` liest diese Zusicherung am Typ ab, statt sie über einen Satzraum zu argumentieren.
5. **Der Befund-Katalog spricht nur über Jetnitys eigenen Datenstand.** 33 Aussagen über Zeitraum, Etappen, Reisende, Dokumentstand und Route, jede mit einer aus der Projektion **berechneten** Bedingung. „Für diese Person ist in Jetnity noch kein Reisedokument hinterlegt" ist nachrechenbar und von Jetnity getragen; „Du brauchst ein Reisedokument" wäre eine amtliche Anforderung und steht in keinem Eintrag – ein Test prüft die Formulierungen des ganzen Katalogs dagegen. `angeboteneBefunde()` berechnet je Reise das **Angebot**; gewählt werden darf nur ein Paar aus Schlüssel **und** Bezug, das darin steht, damit ein wahrer Satz nicht an der falschen Etappe landet. Dasselbe Angebot steht in der Nutzlast: Das Modell wählt aus einer Liste, die es sieht.
6. **Amtliche Lagen laufen ausschliesslich über `amtlicheHinweise`.** Sieben geschlossene Aussageschlüssel, jeder an den geprüften Zustand des Bezugs gebunden – `ergebnis`, `frische`, `fehlendeAngaben`. Die drei `geprueft_*`-Schlüssel setzen `belegt` **und** das passende Ergebnis voraus, die übrigen das Gegenteil: „nicht geprüft" über eine geprüfte Lage ist genauso falsch wie umgekehrt. Kein Schlüssel passt auf jeden Zustand. Die Bindung benutzt die maschinenlesbare Identität aus `BegleiterBezug` (`requirementType`, `scope`, `visaMode`, `ergebnis`, `frische`, `fehlendeAngaben`), **nie** den lokalisierten Anzeigetext – eine Copy-Änderung darf keine Wahrheitsentscheidung verschieben.
7. **`lib/reisebegleiter/pruefung.ts` prüft genau drei Dinge, alle gegen berechnete Wahrheit und keines gegen Sprache:** ob jeder genannte Bezug im Kontext existiert; ob jeder gewählte Befund mit seinem Bezug im berechneten Angebot steht; ob jede amtliche Aussage zum geprüften Zustand ihres Official-Bezugs passt. Eine Prüfung auf Preis, Link, Buchungszustand, behauptete Änderung oder erfundene Anforderung gibt es nicht mehr – nicht gelockert, sondern gegenstandslos: Was unaussprechbar ist, braucht keine Prüfung.
8. **Die Produktform ist Auswahl, nicht Formulierung – und das ist eine Produktentscheidung.** Der Reisebegleiter beantwortet eine Frage, indem er aus Jetnity-eigenen Aussagen auswählt und ordnet. Für die Wahrheitsklasse „Generated Suggestion" ist das die ehrliche Bauform, aber der Nutzen hängt damit an der **Katalogbreite** statt an der Sprachfähigkeit des Modells, und ob das genügt, ist **nicht gemessen** – der einzige Messpunkt ist der noch offene bezahlte Preview-Aufruf. Der Product Owner hat diese Form am 17. September 2026 ausdrücklich **für Preview/Development freigegeben**: zur Messung und Validierung dieser Architektur, nicht als Ausweitung der Wahrheitsbefugnis. Mehr Nutzen heisst künftig mehr Katalog – jede neue Aussage ist eine Jetnity-Formulierung mit nachrechenbarer Bedingung, teurer als ein Prompt, aber überprüfbar. Frei formulierte Prosa bleibt geschlossen; sie wäre erst verantwortbar mit geprüfter amtlicher Wahrheit (echter Requirements-Provider) **und** einer Wahrheitsprüfung, die nicht über einen Satzraum argumentiert.
9. **Ob eine Lage geprüft ist, sagt allein Jetnity.** Eine Aussage über die *Herkunft* der Wahrheit („gilt als geprüft") hebt `unknown` auf `current`. Früher war das ein Wortfilter; heute kann das Modell sie überhaupt nicht formulieren, weil es kein Textfeld hat. Der Prüfstand steht ausschliesslich in Jetnity-eigener Copy.
...
11. Die Kostenintegrität hängt an zwei Zahlen statt an einer Annahme: `BEGLEITER_GRENZEN.eingabeZeichen` (24 000) begrenzt Systemregeln plus Frage, `BEGLEITER_GRENZEN.ausgabeTokens` (1600) liegt weit unter dem reservierten Ausgabebudget. `lib/reisebegleiter/kosten.test.ts` rechnet den schlechtesten Fall je Modell gegen die Reservierung.
12. Eine Reise oberhalb der Eingabegrenze bekommt keine gekürzte Wahrheit, sondern keine Auskunft.
13. Nur Konto-Reisen. Bei einer Gastreise liegt der Reisegraph im Browser und trägt Reisenden-, Staatsangehörigkeits- und Dokumentkontext; diesen Kontext vom Client als Wahrheit anzunehmen, um ihn an ein Modell zu geben, ist der falsche erste Schritt. Gastreisen bleiben unverändert planbar und änderbar.
14. Official-, Safety- und Seasonal-Lagen entstehen über `requirementsLokalFuerReise()`, `safetyLokalFuerReise()` und `seasonalLokalFuerReise()` – dieselben provider-freien Auswertungen, die die Oberfläche ohne Provider-Lage benutzt. Eine Frage an den Reisebegleiter löst keinen Provider-Abruf aus.
15. Die Fläche ist eine eingeklappte Karte in der Reiseübersicht: kein schwebender Chat, kein gleichrangiger Hauptbereich, kein Verlauf, kein Aufruf beim Mounten. `ARBEITSBEREICHE` bleibt unverändert. Die Kopie der Fläche benennt die Herkunft der Sätze; eine Ladezeile, die „formuliert" verspricht, wäre eine falsche Aussage über den Text.

**Überholte Fassungen – Historie, nicht aktueller Vertrag.** Die Punkte 4 bis 7 hatten bis Runde 9 eine andere Gestalt, und alles daran ist ersetzt. Das Ausgabeschema trug vier Felder mit Freitext (`antwort`, `unsicherheiten`, `naechsteSchritte`, `bezuege`); Betrag und Link in diesem Freitext wurden abgelehnt statt entfernt; `pruefung.ts` las Buchungs- und Änderungsbehauptungen aus dem Text; und eine harte amtliche Aussage wurde satzweise über **Modalität × Bereich × Vorbehalt** erkannt und an einen genannten, geprüften Official-Bezug desselben Anforderungstyps gebunden. Dazu kamen nacheinander eine Sprachfestlegung, eine Fremdwortliste und schliesslich eine Erlaubnisliste über einem geführten Wortschatz (`lib/reisebegleiter/wortschatz.ts`, entfernt).

Diese Bauform ist aufgegeben, nicht verfeinert. Sie behauptete stets, **kein aus einer erlaubten Wortmenge bildbarer Satz** sei eine amtliche Aussage – eine Behauptung über einen unendlichen Satzraum aus einem endlichen Wortschatz. Sprache komponiert; die Behauptung ist nicht belegbar, nur wiederholt widerlegbar, und sie wurde achtmal widerlegt. Die Nachträge unten halten die einzelnen Runden fest, damit die Reihe nachvollziehbar bleibt; als Vertrag gilt ausschliesslich die Liste oben.
