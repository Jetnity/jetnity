...
5. `result` und `visaMode` müssen zusätzlich zueinander konsistent sein. Widerspruch, ohne dass eine Seite gewinnt: `required + visa_exempt` sowie `not_required + visa_on_arrival|electronic_visa|visa_before_travel`. Die Evaluation wird auf `result='unknown'`, `visaMode='unknown'`, `status='unknown'` degradiert und bleibt nicht `current`. `conditional` und `unknown` erzeugen keine erfundene Gewissheit und gelten nicht als Widerspruch.
6. eTA bleibt eigener Requirement-Typ und wird nicht als `electronic_visa` umetikettiert. Multi-Citizenship / Multi-Document bleibt 1:n. `requirementsProviderAus()` bleibt `null`. S4-R1 Timeout/Abort/Kill-Switch/Freshness bleiben unverändert.
...
4. Destination Official und Transit Official bleiben getrennte Records, auch bei gleichem Land. Destination bindet nur über `destinationIstOfficialZiel`. Transit behält `scope: 'transit'` und `transitCountryCode`; `boundStageIds` bleibt leer, weil der aktuelle Inputvertrag keine kanonische Transit↔Stage-Relation liefert. Landesgleichheit ist keine Bindung.
...
8. Die serialisierte Projektion enthält keine Pass-/MRZ-/Scan-/Biometrie-/Health-Record-, Auth-/Account-Identifier-, Preis-, Availability-, Booking-URL-, Provider-Raw-, Secret- oder Official-`contextFingerprint`-Felder. Der Fingerprint darf intern zur Sortierung dienen, überschreitet die Allowlist aber nicht.
...
**Nachtrag, 2. September 2026 / Review `5093789177`:** Exact rejected head `42cd37fa`. `contextFingerprint` wurde aus `AssistantOfficialContext` entfernt. Transit-Official wird nicht mehr per `stage.countryCode === transitCountryCode` an Destination-Stages gebunden.
...
4. Das Ausgabeschema hat kein Feld für Anforderung, Preis, Anbieter, Verfügbarkeit, Buchung oder Quelle. Vier Felder: `antwort`, `unsicherheiten`, `naechsteSchritte`, `bezuege`. `additionalProperties: false` plus `strict: true` machen weitere Felder auf der Plattformseite unaussprechbar – **und die serverseitige Prüfung verlässt sich nicht darauf**: `z.strictObject` lehnt ein unerwartetes Feld ab, statt es zu entfernen. Ein Vertrag, der nur auf der Gegenseite gilt, ist hier keiner; und ein Modell, das `lagen` oder `visumErforderlich` mitschickt, hat die Regeln nicht verstanden, weshalb seine Auskunft nicht bereinigt, sondern verworfen wird.
...
6. `lib/reisebegleiter/pruefung.ts` lehnt zusätzlich ab: einen Bezug, den der Kontext nicht kennt; jede Aussage über Buchungszustand (die Projektion trägt keinen – auch „noch nicht gebucht" ist erfunden); eine im Perfekt behauptete Änderung; und unbelegte Gewissheit über amtliche Anforderungen. Letzteres ist ein Wortfilter und nimmt Fehlalarme in Kauf; dieselben Wörter sind in den Systemregeln ausdrücklich verboten, damit ein regelkonformes Modell ihn nicht auslöst.
7. **Gewissheit ist an die amtliche Lage gebunden, auf die sich die Auskunft beruft – und an die Anforderung, über die sie spricht.** Drei Bedingungen, alle zugleich:
   - Die Auskunft muss den tragenden Official-Bezug **nennen**. Die blosse Anwesenheit einer geprüften Lage im Kontext genügt nicht, sonst trüge eine beliebige aktuelle Prüfung den Satz „kein Visum erforderlich".
   - Keiner der von ihr genannten Official-Bezüge darf unbelegt sein.
   - Der tragende Bezug muss **zum Anforderungstyp der Aussage passen**. Eine aktuelle Impfanforderung belegt kein Visum. Dafür trägt `BegleiterBezug` die maschinenlesbare Identität (`requirementType`, `scope`, `visaMode`) unverändert aus der Projektion; sie wird nicht aus dem lokalisierten Anzeigetext zurückgelesen, weil eine Copy-Änderung keine Wahrheitsentscheidung verschieben darf.

   Erkannt wird das satzweise über drei Signale: **Modalität** („muss", „brauchst", „ist erforderlich", „kein", „ohne"), **Bereich** (die geschlossene `OFFICIAL_REQUIREMENT_TYPES`-Taxonomie, geordnet vom speziellen zum allgemeinen, mit `other_entry_requirement` als Auffangbereich für amtlichen Zusammenhang ohne benennbaren Gegenstand) und **Vorbehalt** („ob", „prüfe", „unklar", „nicht geprüft", „möglicherweise"). Erst Modalität und Bereich **ohne** Vorbehalt ergeben eine harte Aussage. Damit gilt die Regel für Behauptung und Verneinung gleichermassen: „Dein Pass muss sechs Monate gültig sein" ist dieselbe erfundene Official-Wahrheit wie „visumfrei".

   Formulierungen, die nicht sagen, worüber sie sprechen – „garantiert", „definitiv", „amtlich bestätigt", „problemlos einreisen", „nicht erforderlich" – haben keinen tragfähigen Bereich und fallen **immer** durch. Enthält ein Text mehrere harte Aussagen, braucht jede ihren eigenen Beleg. Beschreibungen, Fragen und Vorschläge bleiben unverändert zulässig – „Prüfe deine Passgültigkeit in der Reisevorbereitung" ist keine Anforderung, sondern ein Weg in Jetnity.

   Solange kein Requirements-Provider aktiv ist, ist kein Official-Bezug je `belegt`. Diese Prüfung ist deshalb heute keine Auswahl, sondern eine **vollständige Sperre** gegen amtliche Aussagen. Das ist die richtige Richtung: Jetnity hat keine geprüfte amtliche Wahrheit, also darf der Reisebegleiter keine behaupten.
8. **Das Modell schreibt keine Sätze. Es wählt aus.** Sieben Fassungen haben versucht, Freitext des Modells so einzuschränken, dass darin keine amtliche Anforderung ausdrückbar ist: deutsche Muster, mehrsprachige Verbotslisten, eine Spracherkennung, eine Erlaubnisliste über dem Wortschatz, zuletzt eine Erlaubnisliste ohne amtliche Substantive. Jede wurde widerlegt, die letzte mit vier gewöhnlichen, geführten Wörtern: `Du musst ein gültiges Reisedokument haben.`

   Der Fehler lag nie in der jeweiligen Liste. Jede dieser Fassungen behauptete, **kein aus ihrer Wortmenge bildbarer Satz** sei eine amtliche Aussage – eine Behauptung über einen unendlichen Satzraum aus einem endlichen Wortschatz. Sprache komponiert; so eine Behauptung ist nicht belegbar, nur wiederholt widerlegbar. Siebenmal widerlegt ist kein Pech, sondern ein Beweis über die Bauform.

   Darum hat `lib/reisebegleiter/schema.ts` **kein Freitextfeld mehr**: kein `antwort`, kein `unsicherheiten`, kein `naechsteSchritte`. Das Modell wählt Schlüssel aus zwei geschlossenen Katalogen und nennt den Bezug; jeden Satz schreibt Jetnity. Der Satz oben ist nicht abgelehnt – es gibt kein Feld, in das er passt. Damit ist die Zusicherung am Typ ablesbar statt über einen Satzraum argumentiert, und `lib/reisebegleiter/schema.test.ts` liest sie dort ab.

   **Der Katalog der Befunde** (`lib/reisebegleiter/befunde.ts`) spricht ausschliesslich über Jetnitys **eigenen Datenstand** – „Für diese Person ist in Jetnity noch kein Reisedokument hinterlegt", nachrechenbar aus der Projektion. Kein Eintrag behauptet eine Anforderung; ein Test prüft die Formulierungen des ganzen Katalogs dagegen. Gewählt werden darf nur, was `angeboteneBefunde()` für diese Reise vorher als zutreffend berechnet hat, und das Paar aus Schlüssel **und** Bezug muss stimmen – ein wahrer Satz an der falschen Etappe fällt durch. Dieses Angebot steht auch in der Nutzlast: Das Modell wählt aus einer Liste, die es sieht.

   **Amtliche Lagen** laufen weiter ausschliesslich über `amtlicheHinweise` mit sieben Aussageschlüsseln, gebunden an `ergebnis`, `frische` und `fehlendeAngaben` des Bezugs (`lib/reisebegleiter/aussagen.ts`).

   **Der Preis ist eine Produktänderung und gehört dem Product Owner.** Der Reisebegleiter formuliert nicht mehr frei; er wählt aus Jetnity-eigenen Aussagen aus und ordnet sie. Für die Wahrheitsklasse „Generated Suggestion" ist das die ehrliche Bauform – aber es ist eine sichtbare Änderung gegenüber einer frei formulierten Antwort, und der Nutzen hängt jetzt an der Breite des Katalogs statt an der Sprachfähigkeit des Modells. Diese Entscheidung ist nicht vom Agenten zu treffen; siehe Punkt 9.

   **Prosa** (`antwort`, `unsicherheiten`, `naechsteSchritte`) spricht über die Reise – Etappen, Tage, Plan, nächste Schritte in Jetnity. Ihr Register in `lib/reisebegleiter/wortschatz.ts` enthält **kein** amtliches Vokabular: kein Visum, kein Pass, kein Ausweis, keine Impfung, keine Versicherung, kein Nachweis, keine Pflicht, kein „erforderlich", kein „notwendig". Ohne Gegenstand gibt es keine amtliche Aussage – unabhängig von Sprache, Modalität und Paraphrase. Deutsche Flexion und Komposita lösen auf; jeder Kompositumsteil braucht vier Zeichen und einen geführten Stamm. Zwei frühere Ausnahmen sind entfernt, weil sie Umgehungswege waren: einzelne Buchstaben (eine Folge davon ist für einen Leser ein Satz) und der Kontextzusatz – `kontextwortschatz()` liess `stage.name` und `traveller.label` das Register erweitern, und eine Erlaubnisliste, die der Eingang erweitern kann, ist keine. Eigennamen stehen deshalb in den Bezügen, die Jetnity anzeigt, nicht in der Prosa.

   **Amtliche Lagen** laufen über `amtlicheHinweise`: Das Modell wählt einen Official-Bezug und eine Aussage aus sieben geschlossenen Schlüsseln; den Satz schreibt Jetnity (`lib/reisebegleiter/aussagen.ts`), und `passt()` prüft die Aussage gegen `ergebnis`, `frische` und `fehlendeAngaben` des Bezugs. Die drei `geprueft_*`-Schlüssel setzen `belegt` **und** das passende Ergebnis voraus, die übrigen das Gegenteil – „nicht geprüft" über eine geprüfte Lage ist genauso falsch wie umgekehrt. Kein Schlüssel passt auf jeden Zustand.

   **Die Zusicherung ist ein Test, keine Behauptung.** Die Bereichsmuster aus Punkt 7 sind keine Laufzeitschranke mehr, sondern die Definition dessen, was als amtliche Anforderung gilt; `lib/reisebegleiter/wortschatz.test.ts` prüft jeden geführten Stamm und jedes Paar daraus gegen sie. Trifft eines, ist die Trennung gebrochen und der Test schlägt an – so sind beim Bau dieser Fassung `rückreise`, der Stamm `pass` hinter `passen` und `notwendig`/`nötig` gefunden worden. Eine Prüfung, die zur Laufzeit nie greifen kann, als Schranke stehen zu lassen, hat zweimal den Eindruck erzeugt, Prosa werde semantisch geprüft; genau dieser Eindruck war der Befund.
9. **Ob eine Lage geprüft ist, sagt allein Jetnity.** „gilt als geprüft", „ist amtlich geprüft" und Verwandte sind Aussagen über die *Herkunft* der Wahrheit und damit nie bindbar – ein Modell, das sie macht, hebt `unknown` auf `current`. „noch nicht geprüft" und „geprüfte Lage" bleiben unberührt.
10. Ein Versuch, kein zweiter. Keine Wiederholung, kein Sol→Terra-Nachzug wie bei der Reiseänderung, kein Ausweichen in eine andere semantische Modellfunktion. Kein eigener Router: Das Modell kommt aus `modellZustand()`.
11. Die Kostenintegrität hängt an zwei neuen Zahlen statt an einer Annahme: `BEGLEITER_GRENZEN.eingabeZeichen` (24 000) begrenzt Systemregeln plus Frage, `BEGLEITER_GRENZEN.ausgabeTokens` (1600) liegt weit unter dem reservierten Ausgabebudget. `lib/reisebegleiter/kosten.test.ts` rechnet für jedes zugelassene Modell nach, dass der schlechteste tatsächliche Fall unter der Reservierung bleibt. `Modellanfrage.ausgabeTokens` ist additiv und nach oben durch `MODELL_GRENZEN.ausgabeTokens` gedeckelt.
12. Eine Reise oberhalb der Eingabegrenze bekommt keine gekürzte Wahrheit, sondern keine Auskunft.
...
14. Official-, Safety- und Seasonal-Lagen entstehen über `requirementsLokalFuerReise()`, `safetyLokalFuerReise()` und `seasonalLokalFuerReise()` – dieselben provider-freien Auswertungen, die die Oberfläche ohne Provider-Lage benutzt. Eine Frage an den Reisebegleiter löst keinen Provider-Abruf, keine Suche und keine kommerzielle Anfrage aus.
15. Die Fläche ist eine eingeklappte Karte in der Reiseübersicht: kein schwebender Chat, kein gleichrangiger Hauptbereich, kein Verlauf, kein Aufruf beim Mounten. `ARBEITSBEREICHE` bleibt unverändert.
...
**Nachtrag, 17. September 2026 / Technical-Lead-Re-Review auf `3775d980`:** Zwei Befunde, beide Wahrheitsintegrität. Erstens schaltete eine beliebige geprüfte Official-Lage im Kontext die Gewissheitsprüfung global frei; Punkt 7 ist die Korrektur. Zweitens entfernte die serverseitige Prüfung unerwartete Felder still, statt fehlzuschlagen; Punkt 4 ist die Korrektur (`z.strictObject`). Im selben Review hat der Technical Lead die zuvor blockierte Develop-Anwendung der Migration unabhängig durchgeführt und verifiziert (siehe `docs/ASSISTANT_RUNTIME_1_STATUS_2026-09-17.md` Abschnitt 4).

**Nachtrag, 17. September 2026 / Technical-Lead-Re-Review auf `74577e31`:** Ein dritter Befund derselben Familie. Die Gewissheit war zwar an einen **genannten** Official-Bezug gebunden, aber nicht an dessen Anforderungstyp: Eine geprüfte Impfanforderung hätte „kein Visum erforderlich" getragen. Punkt 7 ist entsprechend erweitert, `BegleiterBezug` trägt dafür die maschinenlesbare Anforderungsidentität. Die drei Befunde haben dieselbe Wurzel – jede Stufe der Bindung musste einzeln erzwungen werden, weil keine allgemeine Regel sie mitbringt.

**Nachtrag, 17. September 2026 / Technical-Lead-Re-Review auf `f46d43a0`:** Ein vierter Befund, wieder eine Stufe weiter. Die Bindung war zwar bereichsgenau, deckte aber nur fünf Bereiche und nur Verneinungen ab; eine behauptete Anforderung („Du brauchst eine Reiseversicherung") in einem der übrigen zehn Bereiche fiel durch das Netz. Punkt 7 arbeitet jetzt satzweise über Modalität, Bereich und Vorbehalt und deckt die geschlossene Taxonomie vollständig ab; ein Test prüft die Abdeckung gegen `OFFICIAL_REQUIREMENT_TYPES`, damit ein künftiger Anforderungstyp nicht stillschweigend ungeschützt bleibt.

**Nachtrag, 17. September 2026 / Technical-Lead-Re-Review auf `4837fc9a`:** Ein fünfter Befund, diesmal nicht am Umfang der Regel, sondern an ihrer Zuständigkeit. Die Wahrheitsschranke las Deutsch, der Prompt erlaubte die Sprache der Frage – also war sie für „You need a visa." nicht zuständig. Punkte 8 und 9 sind die Korrektur: Die Antwortsprache wird geschlossen, und eine Behauptung über die Prüfherkunft wird nie bindbar. Die naheliegende Reparatur – englische Wörter in die Liste – wäre keine gewesen: Sie hätte die Fläche offen gelassen und die nächste Sprache abgewartet.

**Nachtrag, 17. September 2026 / Technical-Lead-Re-Review auf `4ffe3f20`:** Ein sechster Befund, und er hat die Bauart widerlegt, nicht den Umfang. Die Sprachschranke der Vorrunde war eine Heuristik über Markerwörtern plus eine endliche Fremdwortliste; beides liess sich mit „Das ist so: İtalya için vize gerekli.“ aushebeln, und die Prüfung lief ausserdem nur über `antwort`. Punkt 8 ist die Korrektur und dreht die Fragestellung: Erlaubnisliste statt Verbotsliste, über allen Modellfeldern. Damit endet die Reihe der Sprachbefunde, weil sie nicht mehr an der Aufzählung hängt.

**Nachtrag, 17. September 2026 / Technical-Lead-Re-Review auf `5ef78e5c`:** Der achte Befund hat die Prämisse aller Vorfassungen widerlegt, und er brauchte dafür keine Sondersprache: `Du musst ein gültiges Reisedokument haben.`, `Du brauchst ein Dokument.`, `Dein Reisedokument muss gültig sein.` – alle Wörter geführt, alle drei Dokumentenanforderungen. Punkt 8 ist deshalb neu gefasst: kein Freitextfeld mehr, Auswahl statt Formulierung. Die Fälle sind nicht mehr abgelehnt, sie sind nicht darstellbar.

**Nachtrag, 17. September 2026 / Technical-Lead-Re-Review auf `e6606ea8`:** Der siebte Befund hat die Erlaubnisliste als Wahrheitsschranke widerlegt, und zu Recht. Drei Umgehungen waren deterministisch: `notwendig`/`nötig` standen im Register, ohne von `PFLICHTWORT` gebunden zu werden; `istUnbedenklich()` liess jeden Einzelbuchstaben durch; und `kontextwortschatz()` liess einen Etappennamen das Register selbst erweitern. Alle fünfzehn Kombinationen aus diesen drei Fällen und den drei Modellfeldern waren auf dem reviewten Head nachweislich zulässig. Punkt 8 ist deshalb keine siebte Fassung der Liste, sondern eine Trennung der Kanäle.

**Bilanz der acht Runden:** Fünf Korrekturen haben den Geltungsbereich erweitert – vom Kontext auf den genannten Bezug, auf den Anforderungstyp, auf die Gesamttaxonomie und die Aussagerichtung, dann auf die Sprachfläche. Die sechste tauschte Verbots- gegen Erlaubnisliste, die siebte nahm das amtliche Vokabular aus der Erlaubnisliste. Alle sieben blieben dieselbe Bauform: eine Behauptung darüber, welche Sätze aus einer Wortmenge bildbar sind. Die achte gibt die Bauform auf.

Die Lehre für vergleichbare Wahrheitsschranken: Solange eine Wahrheitsaussage in einem Freitextfeld **ausdrückbar** ist, ist jede Prüfung darüber eine Näherung, und keine Verfeinerung der Wortmenge macht daraus einen Beweis. Die tragfähige Frage ist nicht, wie man den Text prüft, und auch nicht, welche Wörter er enthalten darf, sondern **ob es das Feld geben muss**. Eine Zusicherung, die man an einem Typ ablesen kann, ist eine; eine, die man über einen Satzraum argumentieren muss, ist eine Hoffnung.

9. **Offene Produktfrage an den Product Owner: Auswahl statt Formulierung.** Punkt 8 löst die Wahrheitsfrage, verändert aber das Produkt. Der Reisebegleiter beantwortet eine Frage jetzt, indem er aus Jetnity-eigenen Aussagen auswählt und ordnet – nicht indem er formuliert. Drei Folgen, die eine Entscheidung brauchen und nicht vom Agenten getroffen werden:

   - **Nutzen.** Ob eine ausgewählte Antwort genügend Wert hat, ist **nicht gemessen**; dafür fehlt der bezahlte Preview-Aufruf. Der Katalog hat heute 33 Aussagen über Zeitraum, Etappen, Reisende, Dokumentstand und Route.
   - **Wachstumspfad.** Mehr Nutzen heisst mehr Katalog, nicht mehr Sprachfreiheit. Jede neue Aussage ist eine Jetnity-Formulierung mit einer nachrechenbaren Bedingung – teurer als ein Prompt, aber überprüfbar.
   - **Alternative.** Frei formulierte Prosa wäre nur verantwortbar, wenn Jetnity geprüfte amtliche Wahrheit besitzt (echter Requirements-Provider) **und** eine Wahrheitsprüfung, die nicht über einen Satzraum argumentiert. Beides ist heute nicht vorhanden.

   Empfehlung: die Auswahlform für Preview/Development behalten, den bezahlten Aufruf als Messpunkt nutzen und danach über Katalogbreite gegen Sprachfreiheit entscheiden.

**Konsequenzen:** Production-Migration, `JETNITY_MODELL_AKTIV` in Production, Production-OpenAI-Secrets und Production-Aufrufe bleiben geschlossen und brauchen eigene Gates. Ein Gast-Reisebegleiter, ein Gesprächsverlauf, ein Auto-Apply von Vorschlägen, ein Assistant-Zugriff auf Provider-/Commercial-Wahrheit und ein zweiter Kostentopf sind jeweils eigene Slices mit eigener Freigabe. Der Preis der Auswahlform ist der **Umfang** der Auskunft, und er ist nicht gemessen: Der Reisebegleiter kann genau sagen, was in seinem Katalog steht, und sonst nichts. Punkt 9 legt diese Produktfrage dem Product Owner vor. Dafür entfällt eine ganze Fehlerklasse: Erfundene Anforderungen, Preise, Links, Buchungszustände und behauptete Änderungen sind nicht mehr abgelehnt, sondern nicht darstellbar – und die eingestandene Grenze aus ADR-0054 (Verfügbarkeitsbehauptungen in freier Formulierung) ist damit für diesen Weg gegenstandslos geworden.
