-- Jetnity V2 – Phase 2.2: Eindeutigkeit der Reisetage während reise_aendern() aufschieben
--
-- public.reise_aendern() fügt und aktualisiert trip_days, bevor entfernte Tage
-- gelöscht sind. Gültige Zielgraphen verletzen day_index bzw. day_date deshalb
-- vorübergehend: Tag einschieben, mittleren Tag entfernen, Etappe kürzen,
-- Folgetage neu nummerieren, Kalenderdaten verschieben.
--
-- Unique-Indizes können nicht DEFERRABLE sein. UNIQUE-Constraints schon.
-- Der partielle Index trip_days_datum_eindeutig (WHERE day_date IS NOT NULL)
-- wird durch UNIQUE (trip_id, user_id, day_date) ersetzt. Mehrere NULL-Daten
-- bleiben zulässig: PostgreSQL behandelt NULL in UNIQUE als verschieden.
--
-- INITIALLY IMMEDIATE: direkte Einzelwrites prüfen weiter sofort.
-- reise_aendern() setzt die beiden Bedingungen in der Transaktion auf DEFERRED
-- und vor dem Rückgabewert wieder auf IMMEDIATE, damit ein ungültiger
-- Zielgraph nicht erst beim Commit auffällt.

alter table public.trip_days
  drop constraint trip_days_index_eindeutig;

alter table public.trip_days
  add constraint trip_days_index_eindeutig
  unique (trip_id, user_id, day_index)
  deferrable initially immediate;

drop index if exists public.trip_days_datum_eindeutig;

alter table public.trip_days
  add constraint trip_days_datum_eindeutig
  unique (trip_id, user_id, day_date)
  deferrable initially immediate;

comment on constraint trip_days_index_eindeutig on public.trip_days is
  'Nummer je Reise. DEFERRABLE, damit reise_aendern() umnummerieren kann, bevor entfernte Tage weg sind.';

comment on constraint trip_days_datum_eindeutig on public.trip_days is
  'Kalenderdatum je Reise. NULL mehrfach erlaubt. DEFERRABLE analog zur Nummer.';
...
comment on function public.reise_aendern(jsonb) is
  'Übernimmt eine geprüfte Änderung an einer bestehenden Reise des aufrufenden Kontos. SECURITY INVOKER, atomisch, prüft trips.revision, ist über last_mutation_id idempotent und überschreibt keine kommerziellen Felder. Bestehende Kennungen unveränderter Zeilen bleiben. Kindzeilen zählen die Fassung in dieser Transaktion nicht ein zweites Mal. Eindeutigkeit von day_index und day_date ist während des Schreibens aufgeschoben und wird vor dem Rückgabewert geprüft.';
