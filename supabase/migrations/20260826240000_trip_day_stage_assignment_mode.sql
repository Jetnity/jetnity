-- TW6-B: Source → Mode. Mode != Provenance.
-- Development-Datenstand vor dieser Migration: 1 Trip, mode/source = legacy_fallback,
-- keine user-Rows. Rename plus Constraint, kein Blind-Backfill von user → explicit.
...
alter table public.trips
  rename column day_stage_assignment_source to day_stage_assignment_mode;

alter table public.trips
  drop constraint if exists trips_day_stage_assignment_source_check;

alter table public.trips
  drop constraint if exists trips_day_stage_assignment_mode_check;

alter table public.trips
  add constraint trips_day_stage_assignment_mode_check
  check (day_stage_assignment_mode in (
    'legacy_fallback',
    'unassigned',
    'single_destination',
    'explicit'
...
comment on column public.trips.day_stage_assignment_mode is
  'Day→Stage Assignment Mode, nicht Provenance. legacy_fallback nur für historischen DB-Bestand. Neue Requests: unassigned, single_destination oder explicit.';
