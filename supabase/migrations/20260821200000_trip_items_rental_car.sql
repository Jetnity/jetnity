-- Jetnity V2 – Rental Car Foundation
--
-- Mietwagen ist kein Transfer. Neuer kind-Wert rental_car auf trip_items.
-- Abholung/Rückgabe nutzen die vorhandenen Ortsfelder. Wenige optionale
-- Mietwagenspalten statt metadata-JSON oder 1:1-Tabelle.
--
...
-- Handels-, Mobilitäts- oder Mietwagenfelder; bestehende Spalten bleiben stehen.

alter table public.trip_items
  add column if not exists rental_supplier text,
  add column if not exists vehicle_class text,
  add column if not exists transmission text,
  add column if not exists rental_evidence text;

alter table public.trip_items
  drop constraint if exists trip_items_kind_werte,
  drop constraint if exists trip_items_booking_nur_kommerziell,
  drop constraint if exists trip_items_mobility_nur_transfer,
  drop constraint if exists trip_items_rental_nur_mietwagen,
  drop constraint if exists trip_items_rental_supplier_laenge,
  drop constraint if exists trip_items_vehicle_class_werte,
  drop constraint if exists trip_items_transmission_werte,
  drop constraint if exists trip_items_rental_evidence_werte;

alter table public.trip_items
  add constraint trip_items_kind_werte
    check (kind in ('flight', 'stay', 'activity', 'transfer', 'rental_car', 'note')),
  add constraint trip_items_booking_nur_kommerziell
    check (booking_status = 'unconfirmed' or kind in ('flight', 'stay', 'transfer', 'rental_car')),
  add constraint trip_items_mobility_nur_transfer
    check (
      kind in ('transfer', 'rental_car')
      or (
  ...
  add constraint trip_items_rental_nur_mietwagen
    check (
      kind = 'rental_car'
      or (
        ...
        and rental_evidence is null
      )
    ),
  add constraint trip_items_rental_supplier_laenge
    check (rental_supplier is null or char_length(btrim(rental_supplier)) between 1 and 120),
  add constraint trip_items_vehicle_class_werte
    check (
      vehicle_class is null
      or vehicle_class in ('economy', 'compact', 'intermediate', 'fullsize', 'suv', 'van', 'luxury')
    ),
  add constraint trip_items_transmission_werte
    check (transmission is null or transmission in ('automatic', 'manual')),
  add constraint trip_items_rental_evidence_werte
    check (rental_evidence is null or rental_evidence = 'user');

-- Transfer-spezifische Felder dürfen auf einem Mietwagen nicht stehen.
alter table public.trip_items
  drop constraint if exists trip_items_mobility_modus_nur_transfer;

alter table public.trip_items
  add constraint trip_items_mobility_modus_nur_transfer
    check (
      kind = 'transfer'
      or (
        mobility_mode is null
...
comment on column public.trip_items.rental_supplier is
  'Vermieter als Nutzerfakt. Nicht der kommerzielle Such-Provider. Null bei Nicht-Mietwagen.';
comment on column public.trip_items.vehicle_class is
  'Fahrzeugklasse nur wenn bekannt. Keine erfundene Klasse.';
comment on column public.trip_items.transmission is
  'Getriebe nur wenn bekannt. Kein erfundener Standard.';
comment on column public.trip_items.rental_evidence is
  'Herkunft der Mietwagenfakten. In dieser Foundation nur user. Der Browser darf keine Providerquelle setzen.';
...
comment on function public.reise_anlegen(jsonb) is
  'Legt eine Reise samt Etappen, Tagen und Planpunkten für das aufrufende Konto an. Idempotent über trips.client_ref. SECURITY INVOKER. Schreibt optionale Ortsreferenzen, Mobilitäts- und Mietwagenfakten. Übernimmt einen manuellen Buchungsstatus nur als user, niemals als provider. Gebucht nur für flight, stay, transfer und rental_car.';
