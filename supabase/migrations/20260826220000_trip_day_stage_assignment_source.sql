-- TW6-B: Day→Stage Assignment Source / Mode.
-- Neue Multi-Ziel-Reisen ohne Aufenthaltsentscheidung bleiben ehrlich unassigned.
-- Altbestand erhält legacy_fallback und behält den proportionalen Fallback.
-- Client darf user / legacy_fallback nicht frei setzen, um Server-Truth zu umgehen.
...
alter table public.trips
  add column if not exists day_stage_assignment_source text not null default 'legacy_fallback';

alter table public.trips
  drop constraint if exists trips_day_stage_assignment_source_check;

alter table public.trips
  add constraint trips_day_stage_assignment_source_check
  check (day_stage_assignment_source in (
    'legacy_fallback',
    'unassigned',
    'single_destination',
    'user'
...
comment on column public.trips.day_stage_assignment_source is
  'Herkunft der Day→Stage-Zuordnung. legacy_fallback = historischer proportionaler Vertrag; unassigned = mehrere Ziele ohne Nutzerzuordnung; single_destination = ein Ziel; user = reserviert für spätere explizite Bestätigung. Client darf user nicht setzen.';
...
begin
  ...
  if _claimed_source = 'user' then
    _claimed_source := null;
  end if;
  ...
  elsif _claimed_source = 'unassigned' then
    _assignment_source := 'unassigned';
  elsif _claimed_source = 'legacy_fallback' then
    _assignment_source := case when _has_day_position then 'legacy_fallback' else 'unassigned' end;
  elsif _claimed_source = 'single_destination' then
    _assignment_source := 'unassigned';
  elsif _claimed_source is null then
    _assignment_source := case when _has_day_position then 'legacy_fallback' else 'unassigned' end;
  else
    _assignment_source := 'unassigned';
  end if;
