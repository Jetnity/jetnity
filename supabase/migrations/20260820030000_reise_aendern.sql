-- Jetnity V2 – Phase 2.2: atomische Änderung einer bestehenden Reise
--
-- Ablauf in der Anwendung: vertrauenswürdige Reise laden → Modelloperationen
-- deterministisch anwenden → prüfen → Vorschau → Bestätigung. Diese Funktion
-- ist nur der letzte Schritt, und nur für Reisen im Konto.
--
-- Eigenschaften:
--
--   · SECURITY INVOKER – RLS bleibt die Eigentumsprüfung.
--   · Optimistische Concurrency über trips.revision.
--   · Idempotenz über trips.last_mutation_id: Retry/Doppelklick wenden dieselbe
--     Änderung nicht ein zweites Mal an.
--   · Eine Transaktion: Fehler rollen den ganzen Stand zurück.
--   · Bestehende Kennungen unveränderter Zeilen bleiben; kein Löschen der
--     ganzen Reise.
--   · Kommerzielle Felder (Preis, Anbieter, Buchungslink, External-Ref) kommen
--     nicht aus der Nutzlast. Unveränderte Planpunkte behalten sie, neue bleiben
--     leer.
--
-- Die Nutzlast ist der bereits angewendete Reisegraph, nicht der Modelloutput.
...
comment on function public.reise_aendern(jsonb) is
  'Übernimmt eine geprüfte Änderung an einer bestehenden Reise des aufrufenden Kontos. SECURITY INVOKER, atomisch, prüft trips.revision, ist über last_mutation_id idempotent und überschreibt keine kommerziellen Felder. Bestehende Kennungen unveränderter Zeilen bleiben.';
