-- Jetnity V2 – Trip Coverage & Booking Status
--
-- Provider-neutraler Buchungsstatus auf trip_items. Ein vorhandener Planpunkt
-- ist ausgewählt/geplant, nicht gebucht. Historische Zeilen bleiben
-- unconfirmed. Der Browser kann keine Provider-Quelle setzen.
--
-- Nur Development/Preview/Test anwenden. Nicht auf Production anwenden.

alter table public.trip_items
  add column if not exists booking_status text not null default 'unconfirmed',
  add column if not exists booking_source text,
  add column if not exists booking_confirmed_at timestamptz;

alter table public.trip_items
  drop constraint if exists trip_items_booking_status_werte,
  drop constraint if exists trip_items_booking_source_werte,
  drop constraint if exists trip_items_booking_konsistent,
  drop constraint if exists trip_items_booking_nur_kommerziell;

alter table public.trip_items
  add constraint trip_items_booking_status_werte
    check (booking_status in ('unconfirmed', 'booked')),
  add constraint trip_items_booking_source_werte
    check (booking_source is null or booking_source = 'user'),
  add constraint trip_items_booking_konsistent
    check (
      (booking_status = 'unconfirmed' and booking_source is null and booking_confirmed_at is null)
      or
      (booking_status = 'booked' and booking_source = 'user' and booking_confirmed_at is not null)
    ),
  add constraint trip_items_booking_nur_kommerziell
    check (booking_status = 'unconfirmed' or kind in ('flight', 'stay'));

comment on column public.trip_items.booking_status is
  'unconfirmed = ausgewählt/geplant. booked nur nach ausdrücklicher Nutzerbestätigung. Niemals automatisch aus einem vorhandenen trip_item ableiten.';
comment on column public.trip_items.booking_source is
  'Wer den Status gesetzt hat. In dieser Phase nur user. Eine vertrauenswürdige Provider-Quelle kommt später serverseitig.';
comment on column public.trip_items.booking_confirmed_at is
  'Zeitpunkt der Bestätigung. Null, solange unconfirmed.';
  ...
  select
    ...
    case
      when btrim(coalesce(p.wert ->> 'booking_status', '')) = 'booked'
           and coalesce(nullif(p.wert ->> 'kind', ''), 'note') in ('flight', 'stay')
        then 'booked'
      else 'unconfirmed'
    end,
    case
...
comment on function public.reise_anlegen(jsonb) is
  'Legt eine Reise samt Etappen, Tagen und Planpunkten für das aufrufende Konto an. Idempotent über trips.client_ref. SECURITY INVOKER. Schreibt optionale Ortsreferenzen und übernimmt einen manuellen Buchungsstatus nur als user, niemals als provider.';
