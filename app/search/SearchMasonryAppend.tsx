// app/search/SearchMasonryAppend.tsx
'use client'
...
export default function SearchMasonryAppend({
  containerId,
  baseParams,
  initialCursor,
}: {
  ...
  // Inhalte, die per Portal in die bestehende SSR-Masonry eingefügt werden
  const portalContent = (
    <>
      {items.map((u) => (
        <div key={u.id} className="mb-6 break-inside-avoid">
          <ResultCard upload={u} />
        </div>
      ))}
      {/* Loading-Skeletons beim Nachladen */}
      {loading && Array.from({ length: 6 }).map((_, i) => (
        <div key={`sk-${i}`} className="mb-6 break-inside-avoid">
          <SkeletonCard />
        </div>
      ))}
      {/* Sentinel für Auto-Load */}
  ...
  return (
    <>
      ...
      {/* Portal in die SSR-Masonry */}
      {container ? ReactDOM.createPortal(portalContent, container) : null}
    </>
  )
}
  ...
  return (
    <article className={cn(
      ...
      <div className="relative w-full">
        {/* kein festes Aspect-Ratio → echte Masonry-Höhen */}
        {img ? (
          <Image
            src={img}
            alt={title}
